...
# validar separado	expect(@response_body[0]['token']).to eq(token)
end
...
# validar separado	expect(@response_body['id']).to eq(id.to_i)
end
...
step 'DELETE/job?id=<id>' do |id|
  ...
  DataStore.keep('DELETE_response_body', JSON.parse(response.body))
